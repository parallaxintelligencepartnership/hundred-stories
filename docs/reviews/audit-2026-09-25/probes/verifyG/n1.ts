...

{
  ...
  r.emit(beat('fire.started'));
  const drones = () => r.ctx.oscillators.filter(o => o.frequency.value === 110).length;
  console.log('110 Hz drones after fire.started =', drones());
  r.sound.setEnabled(false); r.sound.setEnabled(true); r.run(30);
  console.log('110 Hz drones after off/on with fire still burning =', drones(), '| tensionLevel =', r.sound.tensionLevel);
}
