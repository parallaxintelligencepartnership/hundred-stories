...
function build(reload: boolean) {
  ...
  ok({ kind: 'shaft.build', shaft: 'standard', x: 20, floorMin: 1, floorMax: 12 });
  ...
  while (w.time.minute < endDay * 1440) {
    ...
    if (!hotelsBuilt && w.stars >= 2) {
      hotelsBuilt = true;
      let n = 0;
      for (let f = 7; f <= 12; f++) for (let x = 30; x + 4 <= 118; x += 4) if (ok({ kind: 'build', room: 'hotelSingle', floor: f, x })) n++;
      console.log(reload ? 'B' : 'A', 'hotels built', n, 'at minute', w.time.minute);
    }
    if (!reload && w.time.minute % 1440 === 300) { const rs=[...w.rooms.values()].filter((r:any)=>r.kind==='hotelSingle'); console.log('day', Math.floor(w.time.minute/1440), 'dirty', rs.filter((r:any)=>r.dirty).length, 'infested', rs.filter((r:any)=>r.infested).length); }
