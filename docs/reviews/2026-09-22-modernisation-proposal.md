...
Each round ends with the itworks closeout as before. Mandate (Matt, 2026-09-22): the implementing session runs all four rounds top to bottom without waiting for a go between them, closing out and deploying each; it does not stop for anything. The store round builds the wrappers to submission-ready with a submission runbook and ends there, because no store account exists yet; pricing is not a concern and the licence is settled. Wall publishes still need a separate explicit yes.
