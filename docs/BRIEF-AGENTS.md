...
- Verification is one pass (Matt, 2026-09-23, hardened the same night: "I can SEE THEY ARE RUNNING TESTS AGAIN"). Write tests alongside the code like a real developer and run only the one test file you are working on (`npx vitest run tests/<path>`), never the full suite, a directory, or a glob, until the end; then run the full `npm test` and `npm run typecheck` exactly once; skip `npm run build` unless you changed build config (the closeout builds once). Tightened 2026-09-23: stacked full runs from several agents made the suite time out on Matt's machine. No sweep, closeout, version bump or deploy inside a package: one closeout and one deploy cover the whole run, and Claude runs the deploy, never Matt.
