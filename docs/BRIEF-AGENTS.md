...
- Verification is one pass (Matt, 2026-09-23, hardened the same night: "I can SEE THEY ARE RUNNING TESTS AGAIN"). Do not run vitest, npm test, tsc or npm run typecheck in any form while you work, not even on your own file; write the code and the tests, then run the full `npm test` and `npm run typecheck` exactly once, at the end; skip `npm run build` unless you changed build config (the closeout builds once). Tightened 2026-09-23: stacked full runs from several agents made the suite time out on Matt's machine. No sweep, closeout, version bump or deploy inside a package: one closeout and one deploy cover the whole run, and Claude runs the deploy, never Matt.
