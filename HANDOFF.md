> **Latest local work — 2026-09-25 (evening):** 0.5.0 shipped, tag `ship-2026-09-25`, main at the merge of `audit-fixes-2026-09-25`. Every finding of the 2026-09-25 audit and of the closeout sweep is closed on a test that fails when its fix is reverted (packages P1 to P13); no finding is open at any severity and nothing is accepted. Record: `SHIPPED.md`, the closing note [docs/reviews/2026-09-25-remediation-closing.md](docs/reviews/2026-09-25-remediation-closing.md), the audit folder `docs/reviews/audit-2026-09-25/` (fix review, browser probe, News phone shot). Tests, typecheck, build, rollback rehearsal and browser probes ran on pi2 in a Node 26 container (recipe in `.itworks/MAP.md`, Environment); nothing ran on the Mac. Device checks still worth a hand: the phone GPU on the tall shafts, the iPhone audio session, Cmd+A on a Mac keyboard; all three are fixed and guarded by tests already. Save format stays 5 (the roach timer is an optional field); a release that raises the number must not ship until the rollback target reads it.
